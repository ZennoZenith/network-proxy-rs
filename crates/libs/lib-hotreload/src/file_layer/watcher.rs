        ...
        move |result: DebounceEventResult| {
            ...
            rt.spawn(async move {
                if let Err(err) = tx.send(result).await {
                    tracing::error!("Failed to send event result: {err}");
                }
            });
        },
...
pub async fn watch<W: Watcher>(
    ...
    while let Some(result) = rx.recv().await {
                ...
                for e in events {
                    if ignore_files {
                            ...
                            .collect::<Result<Vec<_>, _>>()
                        {
                            ...
                            Err(err) => {
                                tracing::error!(
                                    "Failed to check ignore files: {err}"
                                );
                    ...
                    match e.event.kind {
                        ...
                        Modify(kind) => {
                            ...
                            match kind {
                                ...
                                _ => {
                                    let paths =
                                        e.event.paths[0].to_str().unwrap();
                                    tracing::debug!("[UPDATE] {paths}");
                                    files_changed = true;
                                }
                            }
                        }
        ...
        if files_changed && let Err(err) = tx.send(()) {
            tracing::debug!("Failed to broadcast: {err}");
        }
    }
}
